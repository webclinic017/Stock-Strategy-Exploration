...
##########################################################################
Required_Packages = c('tidyverse','installr','psych','quantmod','lubridate','dygraphs','doParallel','XML','earth', 'googledrive','cumstats','dummy','plotly','knitr','xts','grid','gridExtra')
...
if(Initial_Pull){
  ...
  Total_Stocks = bind_rows(Market_Tickers,NASDAQ_Stocks,NYSE_Stocks,AMEX_Stocks) %>%
    filter(!Symbol %in% c("AGFSW","ARII","CORI","DWCH","TACOW","ECYT","ESRX","JTPY","LINDW","OXBRW","PBSK",
                          ...
                          "FSIC","GFA","HTGX","LHO","LKM","NAP","SEP","EIA","MAB","MIW","EMI","NYH")) %>%
...
for(i in 1:length(Tickers)){
  TMP = Combined_Results %>%
    ...
    mutate(Close_50 = rollapply(Close,
                                width = 50,
                                FUN = mean,
                                na.rm = T,
                                fill = NA,
                                align = "right"),
           Close_200 = rollapply(Close,
                                width = 200,
                                FUN = mean,
                                na.rm = T,
                                fill = NA,
                                align = "right"),
           middle = apply(X = cbind(High,Low,Open,Close),MARGIN = 1,FUN = quantile,0.50,na.rm = T),
           lower = apply(X = cbind(High,Low,Open,Close),MARGIN = 1,FUN = quantile,0.25,na.rm = T),
           upper = apply(X = cbind(High,Low,Open,Close),MARGIN = 1,FUN = quantile,0.75,na.rm = T),
           ymin = Low,
           ymax = High,
           Color = ifelse(Close > Open,"Gain","Loss"),
           Date = as_date(Date)) %>%
    filter(Date >= Plot_Date - 100) %>%
    mutate(Type = "Price")
  Volume_TMP = TMP %>%
    mutate(Volume = scales::rescale(Volume,to = c(min(Low),max(High)))) %>%
    select(Date,contains("Volume"),Color) %>%
    mutate(Type = "Volume",
           Volume_50 = rollapply(Volume,
                                width = 50,
                                FUN = mean,
                                na.rm = T,
                                fill = NA,
  ...
  Total_TMP = bind_rows(TMP,Volume_TMP) %>%
    filter(Date >= Plot_Date)
 
  
  p1 = ggplot(Total_TMP,aes(x = Date)) +
    geom_boxplot(data = subset(Total_TMP,Type == "Price"),aes(lower = lower,
                     middle = middle,
                     upper = upper,
                     ymin = ymin,
                     ymax = ymax,
                     color = Color),
                 stat = "identity") +
    geom_line(data = subset(Total_TMP,Type == "Price"),aes(x = Date,y = Close_50),size = 1) +
    geom_line(data = subset(Total_TMP,Type == "Price"),aes(x = Date,y = Close_200),linetype = 2,size = 1) +
    geom_bar(data = subset(Total_TMP,Type == "Volume"),
             aes(y = Volume,fill = Color),stat = "identity") +
    geom_line(data = subset(Total_TMP,Type == "Volume"),
             aes(y = Volume_50),size = 1) +
    scale_x_date(breaks = scales::pretty_breaks(9)) +
    scale_color_manual(values = c("green","red")) +
    scale_fill_manual(values = c("green","red")) +
    coord_cartesian(ylim = c(min(TMP$Low)*0.9,max(TMP$High))) +
    labs(title = paste0(unique(TMP$Stock)," 6 Month Performance (Green = Gain, Red = Loss)"),
         subtitle = "Dashed Line = 200 Day SMA\nSolid Line = 50 Day SMA",
         y = "Price",
         x = "",
         color = "") +
