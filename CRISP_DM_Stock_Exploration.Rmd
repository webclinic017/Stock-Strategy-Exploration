...
##########################################################################
Required_Packages = c('tidyverse','installr','psych','quantmod','lubridate','dygraphs','doParallel','XML','earth', 'googledrive','cumstats','dummy','knitr','xts','reshape2','mboost','glmnet','broom','recipes','caret','cluster','factoextra')
...
if(Initial_Pull){
  ...
  Total_Stocks = bind_rows(Market_Tickers,NASDAQ_Stocks,NYSE_Stocks,AMEX_Stocks) %>%
    filter(!Symbol %in% c("AGFSW","ARII","CORI","DWCH","TACOW","ECYT","ESRX","JTPY","LINDW","OXBRW","PBSK",
                          ...
                          "MMV","EMJ","EVJ","EIO","EVO","EIP","EVP","CTT","ECOM","ECC","ETX","FEI")) %>%
...
```{r Model Exploration,eval = F,include = F}
if(Google_Drive_L){
  File = drive_download(file = "NASDAQ Historical.RDATA",
                        overwrite = T)
  load(File$local_path)
...
Economic_IND = Reference_Economic_Pull()
NA_Func = function(x){!any(is.na(x))}
Economic_DF = Economic_IND %>%
  select_if(NA_Func) %>%
  rename("Date" = date)

## Creating Market Identifier Information
Market_Type_DF = Plot_DF_2 %>%
  group_by(Date) %>%
  summarise(Fear_Direction = mean(Close_Slope_50_Norm,na.rm = T))

Market_Ind_DF = Plot_DF %>%
  group_by(Date) %>%
  summarise(Market_Direction = min(Delta,na.rm = T)) %>%
  mutate(Market_Direction_50 = rollapply(Market_Direction,
                                         width = 50,
                                         FUN = mean,
                                         na.rm = T,
                                         fill = NA,
...
Market_Info = Market_Ind_DF %>%
  left_join(Market_Type_DF,by = "Date") %>%
  left_join(Economic_DF,by = "Date") %>%
  na.omit()
...
Mod_DF = PR_Stage_R2 %>%
  group_by(Stock) %>%
  mutate(Lead_Return = (lead(Close,90) - Close)/Close) %>%
  group_by(Date) %>%
  mutate(Quantile_Return = ecdf(Lead_Return)(Lead_Return),
         Buy = case_when(
           Quantile_Return >= 0.95 ~ 1,
           TRUE ~ 0
         )) %>%
  ungroup() %>%
...
## Feature Selection
TMP = Mod_DF %>%
  select(-c(Lead_Return,Quantile_Return)) %>%
  mutate(But = as.factor(Buy)) %>%
  as.data.frame()
Vars = Boruta(Buy ~ .,
              doTrace = 1,
              data = TMP)
...
Cor_Table = History_Table %>%
  select_if(is.numeric) %>%
  filter(Pcent.Gain != 0)
(Vars = Variable_Importance_Reduction(DF = Cor_Table,Type = "R",Target = "Pcent.Gain"))
mod = lm(Pcent.Gain~.,
      data = Cor_Table)
A = cor(as.matrix(Cor_Table))
