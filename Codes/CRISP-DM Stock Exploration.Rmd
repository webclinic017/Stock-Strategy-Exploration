...
Train = Train_Set_Total_IMP %>%
  ...
  mutate(Buy = case_when(
         ...
         Buy = as.factor(Buy)) %>%
  dplyr::select(-c(Adjust_TMP,Indicator,Max,Stock,Cumulative_Return,Max,W,Date))
(table(Train$Buy))

glm_var = glm(Buy~.,
              data = Train,
              family = "binomial")
p_vals = as.data.frame(coef(summary(glm_var))[,4]) %>%
  mutate(vars = as.character(rownames(.)))
colnames(p_vals)[1] = "P_Value"
p_val_sig = p_vals %>%
  dplyr::filter(P_Value <= 0.05,
                vars != "(Intercept)")

Train = Train %>%
  dplyr::select(p_val_sig$vars,"Buy")
high_cor = findCorrelation(cor(sapply(Train,as.numeric)))
if(!is_empty(high_cor)){
  Train = Train[,-high_cor]
}
lin_comb = findLinearCombos((sapply(Train,as.numeric)))
if(!is_empty(lin_comb$remove)){
  Train = Train[,-lin_comb$remove]
}


DF = Train %>%
  select(-Buy)
YCS_Process = preProcess(DF,method = c("YeoJohnson","center","scale"))
YCS_DF = predict(YCS_Process,DF)
Train_Combined_DF = bind_cols(YCS_DF) %>%
...
Test_Combined_DF = bind_cols(YCS_Test) %>%
  mutate(Buy = Test$Buy) %>%
  dplyr::select(colnames(Train_Combined_DF))
