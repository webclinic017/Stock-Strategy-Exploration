...
##########################################################################
Required_Packages = c('tidyverse','installr','psych','quantmod')
...
sourceDir(paste0(getwd(),'/Functions/'))
```
...
Dump = foreach(i = 1:length(Stocks),
               .inorder = F,
               .packages = c("tidyverse",
                             ...
                             "TTR")) %dopar% {
                               ...
                               if(nrow(TMP) < 500){
                                 ...
                                 TMP$Adj_Auto_Smooth = Spline_Par_Optim(TMP)
                                 TMP = BS_Indicator_Function(TMP,Column = "Adj_Auto_Smooth")
                                 TMP = na.locf(TMP)
                                 TMP = Stat_Appendage_Function(TMP)
                                 
                                 # ggplot(TMP[1:365,],aes(Date,Adjusted)) +
                                 #   geom_line(aes(color = "Actual")) +
                                 #   geom_point(aes(y = Adj_Auto_Smooth,color = "Smoothed"))
                                 TMP$Adj_Auto_Smooth = NULL
                                 TMP
                               }
                             }
# Removing Clusters
registerDoSEQ()
Train = plyr::ldply(Dump,data.frame)
rm(Dump)
