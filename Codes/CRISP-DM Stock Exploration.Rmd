...
##########################################################################
Required_Packages = c('tidyverse','installr','psych','quantmod','lubridate','dygraphs','doParallel','XML','earth', 'lubridate', 'googledrive','cumstats','dummy','caret')
...
Market_DF = Combined_Results %>%
  ...
  na.omit() %>%
  mutate(Indicator = runMax(Cumulative_Return,90),
         Delta = Cumulative_Return - Indicator) %>%
  na.omit() %>%
...
ggplot(Plot_DF,aes(x = Date,y = Cumulative_Return,color = Market_Status)) +
  geom_point() +
...
if(Local_S){
  save(Pool_Results,Reduced_Results,Total_Results,
...
Target = "Buy"
Remove = c("Date","Stock","Indicator","Max","W","Adjust_TMP","Cumulative_Return")
Weights = Train_Set_Total_IMP$W
...
if(Local_S){
  save(Columns,Train_Set_IMP,Train_Set_Total_IMP,
...
Train = Train_Set_Total_IMP %>%
  ...
  mutate(Buy = case_when(
         ...
         Buy = as.factor(Buy)) %>%
  dplyr::select(c(Buy,Columns,W))
  

DF = Train %>%
  select(Columns)
YCS_Process = preProcess(DF,method = c("BoxCox","center","scale"))
YCS_DF = predict(YCS_Process,DF)
PCA_Process = preProcess(YCS_DF,method = "pca")
PCA_DF = predict(PCA_Process,YCS_DF)
FA_Process = psych::fa(YCS_DF,nfactors = 3,fm = "minchi")
FA_DF = as.data.frame(predict(FA_Process,YCS_DF))
LDA_Process = MASS::lda(x = YCS_DF,
                        grouping = Train$Buy)
LDA_DF = as.data.frame(predict(LDA_Process,YCS_DF)$x)
Train_Combined_DF = bind_cols(YCS_DF,PCA_DF,LDA_DF) %>%
  mutate(Buy = Train$Buy)
...
DF_Test = Test %>%
  select(Columns)
YCS_Test = predict(YCS_Process,DF_Test)
PCA_Test = predict(PCA_Process,YCS_Test)
LDA_Test = as.data.frame(predict(LDA_Process,YCS_Test))
Test_Combined_DF = bind_cols(YCS_Test,PCA_Test,LDA_Test) %>%
  mutate(Buy = Test$Buy)

source("//climsidfs07/RefEng/1 Ref. Engineering (SH, Scroll & IPD)/13) Analytics/R Functions/Caret Model Compare.R")
Model_Compare(Train = Train_Combined_DF,
...
ggplot(Test,aes(Date,Adjust_TMP,color = factor(Decision))) +
  geom_point()
...
```{r Training Final Model}
## Bagged FDA, built in caret function, allows for weights
sample = runif(nrow(Train_Combined_DF))
DF_mod = Train_Combined_DF[sample <= 0.05,]

mod = bagFDA(formula = Buy~.,
             data = DF_mod,
             B = 50)
Pred_Test = predict(mod,Test_Combined_DF)
(Acc = sum(Pred_Test == Test_Combined_DF$Buy) / nrow(Test_Combined_DF))
Pred_Train = predict(mod,DF_mod)
(Acc = sum(Pred_Train == DF_mod$Buy) / nrow(DF_mod))
plot(Pred_Test)
plot(Pred_Train)
plot(Train_Combined_DF$Buy)
```
