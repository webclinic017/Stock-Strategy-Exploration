...
Train = Train_Set_Total_IMP %>%
  ...
  select(c(Target,Columns,W,Date)) %>%
  mutate(Buy = as.factor(Buy)) %>%
  filter(chaikin_AD > quantile(chaikin_AD,0.025),
         chaikin_AD < quantile(chaikin_AD,0.975)) %>%
  select(-c(Date,W))
...
(LDA_Mod = MASS::lda(Buy~.^2,
                    data = LDA_DF))
plot(LDA_Mod)

LDA_Vals = as.data.frame(predict(LDA_Mod)$x)
EARTH_DF = LDA_Vals %>%
  cbind(select(Train,contains("Market"),c(Buy,W,Date)))
...
Test = Test %>%
  cbind(as.data.frame(predict(LDA_Mod,Test)$x))
Test = Test %>%
  mutate(Pred = predict(mod,Test,type = "response"),
         Decision = case_when(
           Pred >= 0.75 ~ 1,
           Pred < 0.75 ~ 0
         ))
