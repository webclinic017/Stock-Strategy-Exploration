...
get_account <- function(live = FALSE){
  ...
  #Send Request
  account = httr::GET(url = paste0(url,"/v2/account"), headers)
...
get_positions <- function(ticker = NULL, live = FALSE){
  ...
  #Send Request
  positions = httr::GET(url = paste0(url,"/v2/positions"), headers)
...
get_orders <- function(ticker = NULL, status = "open", from = NULL, silent = FALSE, live = FALSE){
  ...
  if(!is.null(ticker)){       #If the ticker is not null, then return the orders for the tickers that is specified.
    if(!is.null(from)){     #If the from date is given, then request orders from only that date and on, or else get all orders for that ticker.
      orders = httr::GET(url = paste0(url,"/v2/orders?status=",status,"&after=",from,"T09:30:00-04:00"), headers)
      orders = response_text_clean(orders)
      if(length(orders) != 0) orders = dplyr::filter(orders, symbol %in% ticker)
    } else {
      orders = httr::GET(url = paste0(url,"/v2/orders?status=",status), headers)
      orders = response_text_clean(orders)
      if(length(orders) != 0) orders = dplyr::filter(orders, symbol %in% ticker)
    }
...
#'
#' @export
patch_order <- function(order_id = NULL,
                        qty,
                        time_in_force = "day",
                        limit_price = NULL,
                        stop_price = NULL,
  ...
  #Send Request
  orders = httr::PATCH(url = paste0(url,"/v2/orders/",order_id),
                       body = bodyl,
                       encode = "json",
                       headers)
  cat(paste("Order ID", order_id,"was successfully updated."))
  orders = response_text_clean(orders)
  return(orders)
}
#----------------------------------------------------------------------------------------------
...
cancel_order <- function(ticker, order_id = NULL, live = FALSE){
  ...
  if(is.null(open_orders)){
    ...
    cancel = httr::DELETE(url = paste0(url,"/v2/orders/",order_id), headers)
...
get_assets <- function(ticker = NULL){
  ...
  if(is.null(ticker)){
    assets = httr::GET(url = paste0(url,"/v2/assets"), headers)
    assets = response_text_clean(assets)
  } else{
    assets = httr::GET(url = paste0(url,"/v2/assets/",ticker), headers)
    assets = response_text_clean(assets)
  }
  return(assets)
}
...
get_calendar <- function(from = NULL, to = NULL){
  ...
  if(is.null(from) & is.null(to)){  #Check if any dates were given, and if not then return
    calendar = httr::GET(url = paste0(url,"/v2/calendar"), headers)
    calendar =  response_text_clean(calendar)
  } else{
    calendar = httr::GET(url = paste0(url,"/v2/calendar","?start=",from,"&end=",to), headers)
    calendar =  response_text_clean(calendar)
  }
  return(calendar)
}
...
get_clock <- function(){
  ...
  #Send Request
  clock = httr::GET(url = paste0(url,"/v2/clock"), headers)
...
#' @export
get_meta <- function(ticker=NULL, endpoint=NULL, perpage=NULL,version="v2"){
  #Set URL
...
get_historic_info <- function(ticker=NULL,type=NULL,date=NULL){
  ...
  #Set URL
  path_url = get_url_poly()
  full_path_url = paste0(path_url,"/v2/historic/",type,"/",ticker,"/",date,"?apiKey=",Sys.getenv("APCA-LIVE-API-KEY-ID"))
...
get_poly_last_price <- function(ticker = NULL){
  ...
  #Set URL
  path_url = get_url_poly()
  full_path_url = paste0(path_url,"/v2/last/stocks/",ticker,"?apiKey=",Sys.getenv("APCA-LIVE-API-KEY-ID"))
...
get_poly_last_trade <- function(ticker = NULL){
  ...
  #Set URL
  path_url = get_url_poly()
  full_path_url = paste0(path_url,"/v2/last_quote/stocks/",ticker,"?apiKey=",Sys.getenv("APCA-LIVE-API-KEY-ID"))
...
get_poly_ohlc <- function(ticker=NULL, date=NULL){
  ...
  #Set URL
  path_url = get_url_poly()
  full_path_url = paste0(path_url,"/v2/open-close/",ticker,"/",date,"?apiKey=",Sys.getenv("APCA-LIVE-API-KEY-ID"))
