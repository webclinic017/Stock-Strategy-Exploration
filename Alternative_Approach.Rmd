...
TODAY = ID_DF %>%
  filter(Date == max(Date),
         Close_High_PD_Norm < 0,
         Volatility_Close > 0.055)
...
if(Back_Test){
  Dates = unique(PR_Stage_R4$Date)[(length(unique(PR_Stage_R4$Date)) - 365):length(unique(PR_Stage_R4$Date))]
  counter = 0
  p = progress_estimated(length(Dates))
  Delta = as.numeric(max(PR_Stage_R3$Date) - max(PR_Stage_R4$Date))
  for(i in 1:length(Dates)){
    TODAY = ID_DF %>%
      filter(Date == as.character(Dates[i+Delta+5]),
             Close_High_PD_Norm < 0,
             Volatility_Close > 0.055)
    ...
    ## Periodically Printing Historical Return
    if(i %% 15 == 0){
      ## Calculating Historical Return
      print(scales::percent(sum(History_Table$Pcent.Gain)))
    }
  ...
  Explore = History_Table %>%
    ...
    select(Positive,WAD,everything()) %>%
    arrange(WAD)
  COR = cor(Explore)
}
...
Historical_Return = scales::percent((sum(History_Table$Pcent.Gain) * (365 /
