...
if(Back_Test){
  ...
  Dates = unique(PR_Stage_R4$Date)[Start:(Start+252)]
  
  ## Building Initial Models
  Models = Modeling_Function(PR_Stage_R4 = filter(PR_Stage_R4,
                                                  Volatility_Parkinson < 0.75,
                                                  Volatility_Close > 0.075,
                                                  TDI > -25,
                                                  WAD > -30,
                                                  WAD_Delta > 0.05,
                                                  Alpha_Slope < 0.0014,
  ...
  for(i in 1:length(Dates)){
    ...
    if(nrow(TODAY) > 0){
      Preds = Prediction_Function(Models,
                                  ...
                                  FinViz = F)
      RESULT = Preds$RESULT %>%
        filter(Volatility_Parkinson < 0.75,
               Volatility_Close > 0.075,
               CMO < 70,
               TDI > -25,
               WAD > -30,
               ...
               Ultimate_Oscillator < 45,
               CLV < 0.70,
               Market_Delta_50 > -0.065,
               Market_Delta_50 < -0.0125,
               Alpha_Slope < 0.0014,
               Close_PD > 0.0025,
               Volume_PD_Norm < 2,
               Close_Slope_200_Norm > 0)
...
Explore = History_Table %>%
  ...
  select(Good,Pcent_Adj,Positive,Ultimate_Oscillator,everything())

COR = cor(Explore)

Method_Refine = rpart(Good~.,
                      data = select(Explore,
