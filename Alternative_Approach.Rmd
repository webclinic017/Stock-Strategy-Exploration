...
## Removing Junk Stocks
# Periods of Bad Data (OHLCV == 0)
# Decent Liquidity (median Volume >= 40k)
# Decent Value (median Close * median Volume >= 20mil)
CHECK = PR_Stage_R2 %>%
  group_by(Stock) %>%
...
Explore = History_Table %>%
  ...
  select(Positive,WAD,WAD_Delta,everything())
COR = cor(Explore)
