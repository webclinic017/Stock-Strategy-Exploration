...
Required_Packages = c("speedglm",'tidyverse','installr','psych','quantmod','lubridate','dygraphs','doParallel','XML', 'googledrive','cumstats','dummy','knitr','xts','reshape2','mboost','glmnet','broom','recipes','caret','cluster','factoextra',"HiClimR","rpart","rpart.plot","caret","AlpacaforR",'doSNOW','ranger','roll')
...
Alpha_Beta_Function = function(DF,Col,Ret){
  form = paste0(Col,"~Total_Alpha")
  mod = lm(formula(form),
           data = as.data.frame(DF))
  Alpha = as.numeric(coef(mod)[1])
  Beta = as.numeric(coef(mod)[2])
  Cor = as.numeric(summary(mod)$r.squared)
  if(Ret == "A"){
    return(Alpha)
  }else if(Ret == "B"){
    return(Beta)
