...
save(RESULT,FUTURES,SHORTS,TODAY,
...
```{r Historical Back Test, echo = F, message=F,warning=F,eval = F}
Dates = unique(PR_Stage_R4$Date)

for(i in (length(Dates)-2000):(length(Dates)-90)){
  TODAY = ID_DF %>%
    filter(Date == as.character(Dates[i+60]),
           Adjusted >= 10,
           MACD > 0)
  
  ## PEriodically Updating Models
  if(i %% 30 == 0 | i == (length(Dates)-2000)){
    Models = Modeling_Function(PR_Stage_R4 = PR_Stage_R4,
  ...
  History_Table = Performance_Function(PR_Stage_R3 = PR_Stage_R3,
                                       ...
                                       Initial_History = ifelse(i == (length(Dates)-2000),T,F),
...
write.csv(History_Table,
          file = "C://users//aayorde//desktop//Backtest.csv")
```
