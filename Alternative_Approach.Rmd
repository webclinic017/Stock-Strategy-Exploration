...
RESULT = TODAY %>%
  ...
  filter(Prob >= Cutoff,
         !str_detect(Stock,"^\\^")) %>%
  select(Stock,Date,Prob,Delta,Future,Adjusted,Names_Profit$Var,Names_Futures$Var) %>%
...
Buy_Point = ID_DF %>%
  ...
  select(-contains(".y")) %>% 
  select(Stock,Date,Adjusted,Pred,ATR,Prob,Delta,Limit_Buy,everything())
colnames(Buy_Point) = str_replace_all(colnames(Buy_Point),".x","")
Examine = Buy_Point %>%
  select(Stock,Date,Adjusted,Pred,ATR,Prob,Delta,Limit_Buy,colnames(Train.PP.2)[!colnames(Train.PP.2) %in% "Target"])
